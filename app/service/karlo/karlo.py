...
class Karlo:
    ...
    def inpaint_image(self, prompt, negative_prompt, image_base64, mask, reference_image_base_64, return_type):

        _url = "https://api.kakaobrain.com/v2/inference/karlo/inpainting"
        _json = {
            "prompt": prompt,
            "negative_prompt": negative_prompt,
            "image" : image_base64, 
            "mask" : mask,
            # "reference_image" : reference_image_base_64, 
            "return_type" : return_type
        }

        print("----_json ::: " + str(_json))

        print(requests.post(_url, json=_json, headers=self.headers).content)
        # return requests.post(_url, json=_json, headers=self.headers).json()
